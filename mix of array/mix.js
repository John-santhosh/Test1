...
let total = type.reduce((prev,curr)=> curr.rating+prev,0)
document.write("<h2>1. Filter the elements of type: ‘dairy’ and calculate averageRatings</h2> <br>")
document.write("<h3>",total/type.length,"</h3>","<br>")

// 2. Filter the elements of type: ‘vegetable’ and calculate averagePrice

let typeOfVeg= superMarket.filter(a=>a.type=="vegetable")
let vegTotal =  typeOfVeg.reduce((prev,curr)=>curr.price+prev,0)
document.write("<h2>2. Filter the elements of type: ‘vegetable’ and calculate averagePrice</h2> <br>")
document.write(`<h3>$ ${Math.floor(vegTotal/typeOfVeg.length)} </h3><br>`)

// 3. Filter the elements of type: ‘fruit’ and add new property ‘isAvailable: true’

let typeOfFruit = superMarket.filter(a=>a.type=="fruit").map(a=>{
    return {
        ...a,
        isAvailable: true
    }
...
let resultOfFruit=[]
for (let each in typeOfFruit){
    resultOfFruit.push(`<h3>product ${[+each+1]}</h3> <br>`)
    // console.log(each)
    for(let deep in typeOfFruit[each]){
        // console.log(deep)
        resultOfFruit.push([deep," : ",typeOfFruit[each][deep], "<br>"])
    }

}
document.write("<h3>3. Filter the elements of type: ‘fruit’ and add new property 'isAvailable: true'</h3>")

document.write(resultOfFruit.join(",").replaceAll(",", ""))
...
let filtered = superMarket.filter(a=>a.type=="vegetable" || a.type=="dairy").map(a=>{
    return{
        ...a,isAvailable: true
    }
})
let vegetable = filtered.filter(a=> a.type=="vegetable").map(a=>{
    return{
        ...a,isAvailable:false
    }
})
// using spread operator to create a new expected result 
let result = {
    ...filtered,...vegetable
}
...
for (let each in result){
    resultFiltered.push(`<h3>Product ${+each+1}</h3>`)
    for(let deep in result[each]){
        resultFiltered.push([deep , " : ", result[each][deep],"<br>"])
    }
}

document.write(resultFiltered.join(",").replaceAll(",",""))